                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center justify-between">
                      <p className="text-gray-600">{order.cafe_name || order.cafe?.name || 'Unknown Cafe'}</p>
                    ...
                    <div className="text-sm text-gray-600">
                      {(order.items || order.order_items || []).length} item{(order.items || order.order_items || []).length !== 1 ? 's' : ''}
                      {(order.items || order.order_items || []).length <= 3 && (
                        <span className="ml-2">
                          {(order.items || order.order_items || []).map(item => item.name || item.menu_item?.name || 'Unknown Item').join(', ')}
                        </span>
                      )}
                    </div>
                  </div>
